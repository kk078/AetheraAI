...
describe("ar_prioritizer", () => {
  it("ranks by score and summarizes buckets", async () => {
    const r = await arPrioritizer.execute({ accounts: [
...
describe("underpayment_detector", () => {
  it("totals recoverable variance", async () => {
    const r = await underpaymentDetector.execute({ lines: [
...
describe("timely_filing_calculator", () => {
  it("flags expired", async () => {
    ...
    const r = await timelyFilingCalculator.execute({ date_of_service: dos, payer_class: "commercial" });
    expect((r.data as any).status).toBe("expired");
  });
});

describe("modifier_recommender", () => {
  it("maps scenario to modifiers", async () => {
    const r = await modifierRecommender.execute({ scenario: { bilateral_procedure: true, side: "left", multiple_procedures: true } });
...
describe("medical_necessity_builder", () => {
  it("rates strong when fully documented", async () => {
    const r = await medicalNecessityBuilder.execute({
...
describe("hcc_gap_finder", () => {
  it("finds unrecaptured HCCs", async () => {
    const r = await hccGapFinder.execute({ prior_year_hccs: ["HCC19", "HCC85"], current_year_dx: ["E11.9"], revenue_per_raf: 10000 });
...
describe("structured_extractor", () => {
  it("extracts email and ssn", async () => {
    const r = await structuredExtractor.execute({ text: "jane@doe.com SSN 123-45-6789", fields: [
...
describe("data_insights", () => {
  it("group_by sum", async () => {
    const r = await dataInsights.execute({ action: "group_by", group_field: "payer", agg_field: "amount", agg: "sum",
