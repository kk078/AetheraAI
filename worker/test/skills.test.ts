...
describe("rcm_kpi_calculator", () => {
  it("computes and grades KPIs", async () => {
    const r = await rcmKpiCalculator.execute({
  ...
  it("fails with no usable inputs", async () => {
    expect((await rcmKpiCalculator.execute({ total_ar: 100 })).success).toBe(false);
  });
});

describe("em_level_advisor", () => {
  it("by time (established)", async () => {
    const r = await emLevelAdvisor.execute({ method: "time", patient_type: "established", total_time_minutes: 35 });
    expect((r.data as any).recommended_code).toBe("99214");
  });
  it("by MDM 2-of-3", async () => {
    const r = await emLevelAdvisor.execute({
  ...
  it("rejects below-threshold time", async () => {
    expect((await emLevelAdvisor.execute({ method: "time", patient_type: "established", total_time_minutes: 5 })).success).toBe(false);
  });
});

describe("patient_cost_estimator", () => {
  it("deductible + coinsurance", async () => {
    const r = await patientCostEstimator.execute({ charge: 300, allowed_amount: 200, deductible_remaining: 50, coinsurance_rate: 0.2 });
    expect((r.data as any).estimated_patient_responsibility).toBe(80);
    expect((r.data as any).contractual_adjustment).toBe(100);
  });
  it("caps at OOP max", async () => {
    const r = await patientCostEstimator.execute({ charge: 1000, allowed_amount: 1000, deductible_remaining: 1000, oop_max_remaining: 250 });
